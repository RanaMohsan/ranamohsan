(() => {
  ...
  function populateLookups(){
    $('vendor').innerHTML='<option value="">Select vendor (optional for draft)</option>'+(vendors.length?vendors.map(x=>`<option value="${x.vendorId}">${esc(x.vendorCode)} - ${esc(x.vendorName)}</option>`).join(''):'');
  ...
  async function prepareNewInvoice(){
    ...
    try{ await loadLookups(); populateLookups(); fillVendor(); fillProduct(); renderLines(); dirtyVersion=0; savedVersion=0; msg('status','New purchase invoice is ready. It will save automatically after a line is added.',true); }
  ...
  async function loadInvoice(){
    try{
      ...
      $('vendor').value=Number(header.vendorId)>0?String(header.vendorId):''; $('vendorInvoiceNo').value=header.vendorInvoiceNo||''; $('invoiceDate').value=dateOnly(header.invoiceDate); $('paidAmount').value=num(header.paidAmount).toFixed(2); $('remarks').value=header.remarks||''; $('documentNo').textContent=header.invoiceNo||`#${invoiceId}`; $('documentTitle').textContent=`${isPosted?'Purchase Invoice':'Open Purchase Invoice'} ${header.invoiceNo||''}`.trim();
  ...
  function setOpenStatus(label='Open'){ $('documentStatus').textContent=label; $('documentStatus').className='bc-status-pill draft'; $('factStatus').textContent=label==='New'?'New':'Open'; }
  function setReadOnlyMode(){ isReadOnly=true; ['vendor','vendorInvoiceNo','invoiceDate','paidAmount','remarks'].forEach(id=>$(id).disabled=true); $('lineEntrySection').classList.add('read-only'); $('lineEntrySection').querySelectorAll('input,select,button').forEach(el=>el.disabled=true); $('saveDraftBtn').hidden=true; $('postBtn').hidden=true; $('printBtn').disabled=false; }
  ...
  function fillProduct(){ const product=products.find(x=>Number(x.productId)===Number($('product').value)); if(!product)return; $('unitCost').value=num(product.purchasePrice).toFixed(2); $('taxPercent').value=num(product.taxPercent).toFixed(2); }
  ...
  function renderLines(){
    const body=$('linesBody'); if(!lines.length){body.innerHTML='<tr><td colspan="9" class="bc-empty-row">No invoice lines.</td></tr>';renderSummary();return;}
    body.innerHTML=lines.map((line,index)=>{const calc=postedHeader?{taxAmount:num(line.taxAmount),lineTotal:num(line.lineTotal)}:calculateLine(line);return `<tr><td>Item</td><td>${esc(line.productCode||'')}</td><td>${esc(line.productName)}</td><td class="number">${num(line.quantity).toLocaleString()}</td><td class="number">${money(line.unitCost)}</td><td class="number">${num(line.taxPercent).toFixed(2)}</td><td class="number">${money(calc.taxAmount)}</td><td class="number">${money(calc.lineTotal)}</td><td class="bc-row-action">${isReadOnly?'':`<button type="button" class="bc-icon-button danger" data-remove="${index}" title="Remove line">×</button>`}</td></tr>`;}).join('');
    body.querySelectorAll('[data-remove]').forEach(button=>button.addEventListener('click',()=>{lines.splice(Number(button.dataset.remove),1);renderLines();markDirty();})); renderSummary();
  }
  ...
  function saveDraft(showMessage){
    clearTimeout(saveTimer); if(isReadOnly||isPosting)return Promise.resolve(true); if(!invoiceId&&!lines.length){if(showMessage)msg('status','Add at least one line before saving the draft.',false);return Promise.resolve(false);} if(dirtyVersion===savedVersion&&invoiceId>0){if(showMessage)msg('status',`Purchase invoice ${$('documentNo').textContent||''} is already saved as Open.`,true);return Promise.resolve(true);}
    saveQueue=saveQueue.catch(()=>true).then(async()=>{const versionBeingSaved=dirtyVersion;setOpenStatus('Saving…');try{const result=await api.post('/api/purchase-invoices/draft',draftPayload());invoiceId=Number(result.purchaseInvoiceId||invoiceId||0);if(invoiceId){history.replaceState(null,'',`/purchase-invoice-card.html?id=${encodeURIComponent(invoiceId)}`);$('documentNo').textContent=result.invoiceNo||$('documentNo').textContent;$('documentTitle').textContent=`Open Purchase Invoice ${result.invoiceNo||''}`.trim();}if(dirtyVersion===versionBeingSaved)savedVersion=versionBeingSaved;setOpenStatus('Open');if(showMessage)msg('status',`Purchase invoice ${result.invoiceNo||''} saved as Open.`,true);return true;}catch(error){setOpenStatus(invoiceId?'Open':'New');msg('status',`Auto-save failed: ${error.message}`,false);return false;}});return saveQueue;
  }
