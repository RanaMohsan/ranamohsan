  ...
  try{
    ...
    usersBody.innerHTML=users.length?users.map(u=>{
      ...
      return `<tr onclick="openUser(${id})" class="click-row">
        ...
        <td><code>${val(u,'userName','UserName')||'-'}</code></td>
    ...
    }).join(''):'<tr><td colspan="9" class="muted">No users found.</td></tr>';
  }catch(e){ usersBody.innerHTML=`<tr><td colspan="9" class="bad">${e.message}</td></tr>`; }
}
