...
function isGmail(x){ const v=String(x||'').trim().toLowerCase(); return isEmail(v) && (v.endsWith('@gmail.com') || v.endsWith('@googlemail.com')); }
...
function newUser(){
  ...
  userId.value=0; cardMode.textContent='New User'; userName.value=''; displayName.value=''; email.value=''; phoneNumber.value=''; password.value=''; password.type='password'; togglePasswordBtn.textContent='Show'; passwordHelp.textContent='Enter a password to create this user.'; isActive.value='true'; isCompanySuperAdmin.checked=false; allowDesktopAccess.checked=true;
...
async function loadUser(id){
  ...
  allowDesktopAccess.checked=true;
  try{
    const desktop=await api.get('/api/users/'+id+'/desktop-access');
    allowDesktopAccess.checked=!!(desktop.isAllowed ?? desktop.IsAllowed ?? true);
    if(desktop.companyBlocked || desktop.CompanyBlocked){
      allowDesktopAccess.disabled=true;
      allowDesktopAccess.title='InterNex owner has blocked the desktop application for this company.';
    }else{
      allowDesktopAccess.disabled=false;
      allowDesktopAccess.title='';
    }
  }catch{
    allowDesktopAccess.checked=true;
    allowDesktopAccess.disabled=false;
...
email?.addEventListener('input',()=>{ const gmail=email.value.trim(); if(gmail) userName.value=gmail; });
userName?.addEventListener('input',()=>{ if(!email.value.trim() && isGmail(userName.value)) email.value=userName.value.trim(); });
...
async function saveUser(){
  try{
    ...
    const gmail=(email.value.trim()||userName.value.trim());
    if(!isGmail(gmail)){msg('userStatus','Company users must use a real Gmail address (@gmail.com). Login OTP is sent to that inbox.',false);email.focus();return;}
    email.value=gmail; userName.value=gmail;
    ...
    if(savedId>0){
      try{
        await api.post('/api/users/'+savedId+'/desktop-access',{
          isAllowed:!!allowDesktopAccess.checked,
          userName:(userName.value.trim()||email.value.trim())
        });
      }catch(desktopError){
        msg('userStatus',(r.message||'User saved.')+' Desktop access could not be updated: '+desktopError.message,false);
      }
    }
  }catch(e){msg('userStatus',e.message,false)}
}
