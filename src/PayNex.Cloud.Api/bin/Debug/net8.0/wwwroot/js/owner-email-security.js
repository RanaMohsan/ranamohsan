(()=>{
  ...
  async function ensureOwner(){
    ...
    if(!(me.isPlatformOwner||me.IsPlatformOwner)){location.href='/workspace.html';throw new Error('InterNex Owner access is required.');}
    $('who').textContent=`${me.displayName||me.DisplayName||'InterNex Owner'} | Platform Super Admin | Confidential security settings`;
  ...
  async function load(){
    ...
    $('fromEmail').value=x.fromEmail||'';$('fromName').value=x.fromName||'InterNex Cloud ERP';$('smtpHost').value=x.smtpHost||'';$('smtpPort').value=x.smtpPort||587;$('smtpUser').value=x.smtpUser||'';$('enableSsl').checked=x.enableSsl!==false;$('returnDevOtp').checked=!!x.returnDevOtp;$('otpMinutes').value=x.loginOtpExpiryMinutes||10;$('trustedDays').value=x.trustedDeviceDays||30;
