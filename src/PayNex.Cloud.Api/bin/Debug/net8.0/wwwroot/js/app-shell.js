(function(){
  ...
  const png=document.createElement('link'); png.rel='icon'; png.type='image/png'; png.href='/images/favicon.png?v=internex-tab-20260913'; document.head.appendChild(png);
  const ico=document.createElement('link'); ico.rel='shortcut icon'; ico.href='/images/favicon.ico?v=internex-tab-20260913'; document.head.appendChild(ico);
  ...
  function isActive(path){
    ...
    if(cleanPath === '/bank-accounts.html' && location.pathname.endsWith('/bank-account-card.html')) return true;
    ...
    if(cleanPath === '/platform-portal.html' && (location.pathname.endsWith('/platform-company-card.html') || location.pathname.endsWith('/platform-mobile-app.html') || location.pathname.endsWith('/platform-desktop-app.html'))) return true;
    if(cleanPath === '/platform-subscriptions.html' && (location.pathname.endsWith('/platform-subscription-card.html') || location.pathname.endsWith('/platform-subscription-report.html'))) return true;
  ...
  const title = (document.title || 'InterNex Cloud').replace(/^InterNex\s*/i,'').trim() || 'InterNex Cloud';
  const sidebar = document.createElement('aside');
  sidebar.className = 'app-sidebar';
  sidebar.innerHTML = `
    <div class="brand">
      <div class="brand-logo" aria-hidden="true"></div>
      <div><h2>InterNex</h2><p>SaaS ERP</p></div>
    </div>
    <div class="side-user">
  ...
  // Hide Production/Sandbox chooser until /api/me confirms sandbox is ready for this company.
  try{
    const cachedUser = JSON.parse(localStorage.getItem('paynex_last_user') || '{}');
    if(!(cachedUser.sandboxReady || cachedUser.SandboxReady) && environmentSwitcher){
      environmentSwitcher.hidden = true;
      if(launcher){ launcher.disabled = true; launcher.setAttribute('aria-disabled','true'); }
    }
  }catch{}
  ...
  try{
    if(window.api){
      api.get('/api/me').then(me=>{
        ...
        if(sr) sr.textContent = isOwner ? 'Platform Super Admin • InterNex Cloud ERP' : ([role,company].filter(Boolean).join(' • ') || 'Company workspace');
        if(badge) badge.textContent = isOwner ? 'InterNex Owner' : (env + ' SaaS');
        if(!isOwner) syncEnvironmentPanel(env);
        const sandboxReady = !!(me.sandboxReady || me.SandboxReady);
        if(environmentSwitcher){
          if(isOwner && ((me.companyCode || me.CompanyCode || '') === 'PAYNEX' || !(me.databaseName || me.DatabaseName))){
            // platform-only owner path handled below
          } else if(!sandboxReady){
            setEnvironmentPanelOpen(false);
            environmentSwitcher.hidden = true;
            if(launcher){ launcher.disabled = true; launcher.setAttribute('aria-disabled','true'); }
            if(envMenu){ envMenu.classList.remove('open'); envMenu.setAttribute('aria-hidden','true'); }
          } else {
            environmentSwitcher.hidden = false;
            if(launcher){ launcher.disabled = false; launcher.removeAttribute('aria-disabled'); }
          }
        }
        if(isOwner){
          const menu=document.querySelector('.side-menu');
          ...
          if(menu && !document.getElementById('ownerPortalMenuLink')){
            ...
            const subActive = isActive('/platform-subscriptions.html') ? 'active' : '';
            menu.insertAdjacentHTML('afterbegin', `<div class="side-section owner-section">Owner</div><a id="ownerPortalMenuLink" href="/platform-portal.html" class="${active}"><span class="micon">PN</span><span>Owner Portal</span></a><a id="ownerSubscriptionsMenuLink" href="/platform-subscriptions.html" class="${subActive}"><span class="micon">SUB</span><span>Subscriptions</span></a><a id="ownerEmailSecurityMenuLink" href="/owner-email-security.html" class="${emailActive}"><span class="micon">OTP</span><span>Email & OTP Setup</span></a>`);
          }
          const envLauncher=document.getElementById('paynexEnvLauncher');
          const envMenu=document.getElementById('paynexEnvMenu');
          const environmentSwitcher=document.getElementById('paynexEnvironmentSwitcher');
          const branch=document.getElementById('paynexBranchSelector');
          if(envLauncher) envLauncher.title='InterNex Owner / Environment';
          if(platformOnly){
            setEnvironmentPanelOpen(false);
            if(envLauncher){ envLauncher.disabled=true; envLauncher.setAttribute('aria-disabled','true'); }
            if(environmentSwitcher){ environmentSwitcher.hidden = true; environmentSwitcher.classList.add('owner-environment-locked'); }
            ...
            const ownerPortalPath = location.pathname.endsWith('/platform-portal.html')
              ...
              || location.pathname.endsWith('/platform-desktop-app.html')
              || location.pathname.endsWith('/platform-subscriptions.html')
              || location.pathname.endsWith('/platform-subscription-card.html')
              || location.pathname.endsWith('/platform-subscription-report.html')
  ...
  function enhanceSelect(select){
    ...
    const refresh=()=>{
      if(!select.value){ display.value=''; display.title=''; return; }
      display.value=selectText(select); display.title=display.value;
    };
    select.addEventListener('change',refresh);
    new MutationObserver(refresh).observe(select,{childList:true,subtree:true,attributes:true});
    btn.onclick=()=>{
      const rows=[...select.options].filter(o=>String(o.value||'').trim()!=='').map((o,idx)=>({idx, value:o.value, text:o.textContent.trim()}));
  ...
  if(!document.body.classList.contains('pos-cashier-body')){
    setTimeout(()=>{
  ...
  if(!document.body.classList.contains('pos-cashier-body') && !window.PayNexListFilters && !document.querySelector('script[data-paynex-list-filters]')){
