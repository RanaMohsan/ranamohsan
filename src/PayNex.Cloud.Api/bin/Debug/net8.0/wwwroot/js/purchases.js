(() => {
  ...
  function updateDeleteButton(){
    ...
    if(!btn) return;
    const selected = selectedInvoice();
    btn.disabled = !selected || !isOpenDraft(selected);
  ...
  async function deleteSelected(){
    const selected = selectedInvoice();
    if(!selected){ msg('status', 'Select an Open purchase invoice first.', false); return; }
    if(!isOpenDraft(selected)){ msg('status', 'Only Open or Draft purchase invoices can be deleted.', false); return; }
    const id = Number(selected.purchaseInvoiceId || 0);
    const invoiceNo = selected.invoiceNo || id;
    if(!id){ msg('status', 'Purchase invoice id was not found.', false); return; }
    if(!confirm(`Delete open purchase invoice ${invoiceNo}? This cannot be undone.`)) return;
    try{
      $('deleteInvoiceBtn').disabled = true;
      const result = await api.delete('/api/purchase-invoices/' + id);
      selectedId = 0;
      msg('status', result.message || `Purchase invoice ${invoiceNo} deleted.`, true);
      await loadInvoices();
    }catch(error){
      msg('status', error.message, false);
      updateDeleteButton();
