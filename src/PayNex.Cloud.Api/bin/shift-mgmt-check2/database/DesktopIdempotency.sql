...
IF COL_LENGTH('dbo.SalesReturnOrderHeader','ExternalClientDocumentId') IS NULL
    ALTER TABLE dbo.SalesReturnOrderHeader ADD ExternalClientDocumentId NVARCHAR(80) NULL;
...
IF COL_LENGTH('dbo.SalesReturnOrderHeader','ExternalClientDocumentId') IS NOT NULL
AND NOT EXISTS (
    SELECT 1 FROM sys.indexes
    WHERE name = 'UX_SalesReturnOrderHeader_ExternalClientDocumentId'
      AND object_id = OBJECT_ID('dbo.SalesReturnOrderHeader'))
    CREATE UNIQUE INDEX UX_SalesReturnOrderHeader_ExternalClientDocumentId
        ON dbo.SalesReturnOrderHeader(ExternalClientDocumentId)
        WHERE ExternalClientDocumentId IS NOT NULL;
