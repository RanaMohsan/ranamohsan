(() => {
  ...
  function bindEvents(){
    ...
    $('userRows').addEventListener('click', async event => {
      const copyUserButton = event.target.closest('[data-copy-username-user]');
      if(copyUserButton){
        const userId = Number(copyUserButton.dataset.copyUsernameUser || 0);
        const selectedUser = findCompanyUser(userId);
        const loginName = String(value(selectedUser || {}, 'userName', 'UserName') || '').trim();
        if(!loginName){ setMessage('No user name available to copy.', false); return; }
        try{
          await copyText(loginName);
          setMessage('User name copied. Use this to sign in to InterNex and Mobile App.', true);
        }catch{
  ...
  function prepareNewCompany(){
    ...
    renderRelated([], [], [], 'InterNex owner access is portal-only and is not created inside client company databases.');
    updateFactBoxes();
  ...
  async function loadCompany(){
    ...
    try{
      ...
      renderRelated(users, branches, directory, `${policy} InterNex owner email is portal-only and is not created inside client databases.`);
  ...
  function renderCurrentRelated(){
    ...
    renderRelated(companyUsers(), currentDetails.branches || currentDetails.Branches || [], currentDetails.centralDirectory || currentDetails.CentralDirectory || [], `${policy} InterNex owner email is portal-only and is not created inside client databases.`);
  }
  ...
  function renderRelated(users, branches, directory, securityText){
    ...
    $('userRows').innerHTML = users.length ? users.map(user => {
      if(user.error || user.Error) return `<tr><td colspan="9" class="bc-empty-row bad">${esc(user.error || user.Error)}</td></tr>`;
      const active = toBool(value(user, 'isActive', 'IsActive'));
      const userId = Number(value(user, 'userId', 'UserId'));
      const loginName = String(value(user, 'userName', 'UserName') || '').trim();
      ...
      return `<tr>
        <td>${esc(userId)}</td>
        <td><b>${esc(value(user, 'displayName', 'DisplayName') || loginName)}</b></td>
        ...
        <td><div class="password-table-actions">
          <code class="password-clear-text">${esc(loginName || '—')}</code>
          ${loginName ? `<button type="button" class="success compact-button" data-copy-username-user="${userId}">Copy</button>` : ''}
        </div></td>
        <td><div class="password-table-actions">
    ...
    }).join('') : '<tr><td colspan="9" class="bc-empty-row">No users found.</td></tr>';
